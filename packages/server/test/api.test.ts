...
describe("agents, chats, messages, profiles", () => {
  ...
  test("chats can be renamed and re-profiled over PATCH without dropping the agent", async () => {
    ...
    expect((await readJson<{ chat: { title: string; profileId: string; agentId: string } }>(renamed)).chat).toMatchObject({
      title: "Soil log",
      profileId: "grok",
      agentId: agent.id,
    });
    ...
    expect((await readJson<{ chat: { profileId: string; title: string; agentId: string } }>(switched)).chat).toMatchObject({
      profileId: "fast",
      title: "Soil log",
      agentId: agent.id,
    });
