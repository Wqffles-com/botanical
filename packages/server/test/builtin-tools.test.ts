...
describe("GET /api/tools", () => {
  ...
  test("createApp exposes the same registry the route lists", async () => {
    const config = loadConfig({
      BOTANICAL_PASSWORD: "correct horse",
      BOTANICAL_PROFILES: JSON.stringify([{ id: "mock", name: "Mock", provider: "mock", model: "echo" }]),
    });
    const app = createApp({ config, store: createMemoryStore(), workspaceRoot: scratch() });
    const listed = await app.tools.list();
    expect(listed.find((tool) => tool.id === "code_exec")?.source).toBe("builtin");
    const { token } = await login(app);
    const response = await app.fetch(
      new Request("http://localhost/api/tools", { headers: bearer(token) }),
    );
    const body = await readJson<{ tools: Array<{ id: string }> }>(response);
    expect(body.tools.map((tool) => tool.id)).toEqual(listed.map((tool) => tool.id));
  });
});
