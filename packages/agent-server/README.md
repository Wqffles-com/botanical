...
`packages/providers` should implement `LLMProvider` from `@botanical/agent-runtime`. `packages/tools` can be wrapped with `adaptTool`. Expected built-in names are `web_search`, `web_fetch`, `shell`, `file_read`, and `file_write`. MCP config is `examples/mcp.json` (`stdio`, streamable `http`, or legacy `sse`).

```ts
import { createAgentMessageBus, createBuiltinToolSource, createMcpToolSource, createRuntimeToolSource, staticProfileResolver } from "@botanical/agent-runtime";
